...
class Build : NukeBuild
{
    ...
    Target CreateServices => _ => _
        ...
        .Executes(async () =>
        {
            var services = new[]
            {
                new {Service = "p-config-server", Plan = "standard", InstanceName = "config-server"},
                new {Service = "p-service-registry", Plan = "standard", InstanceName = "eureka"},
                new {Service = "p.mysql", Plan = DbPlan, InstanceName = "mysql"},
            };
            CloudFoundryCreateService(c => c
                .CombineWith(services, (oo, service) => oo
                    .SetService(service.Service)
                    .SetPlan(service.Plan)
                    .SetInstanceName(service.InstanceName)));
            
            await Task.WhenAll(services.Select(x => CloudFoundryEnsureServiceReady(x.InstanceName))) ;
        });
    Target Deploy => _ => _
    ...
    {
        var guid = CloudFoundry($"service {serviceInstance} --guid", logOutput: false).First().Text;
        bool IsCreating()
        {
        ...
        Logger.Info($"Waiting service {serviceInstance} to finish provisioning");
        while (IsCreating())
        {
            await Task.Delay(5000);
        }
        Logger.Info($"Service {serviceInstance} is finished provisioning");
